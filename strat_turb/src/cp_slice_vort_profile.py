...
for i in range(nr):
    ...
    for j in range(num_pt):
        ...
        omega[i] += np.sqrt(ux_xy[k,ind[0][j],ind[1][j]]**2 +\
            uy_xy[k,ind[0][j],ind[1][j]]**2)/r[i]
...
for i in range(nr_plot):
    nr_r[i] = dr*(i+1)
    idx = np.where(r[0]+(i-1)*dr < r < r[0]+i*dr)
    print('Checking indices in omega loop: ',idx)
    nr_omega[i] += np.sum(omega[idx])/len(idx)
for i in range(nr-2):
    #ang_mom[i+1] = (r2om[i+2] - r2om[i])/(r[i]*(r[i+2]-r[i])) + 1/Ro
    ang_mom[i+1] = (nr_r[i+2]**2*(omega[i+2]+1/Ro) -\
        nr_r[i]**2*(omega[i]+1/Ro))/(0.5*dr*(nr_r[i+2]+nr_r[i]))

#ang_mom[0] = ang_mom[1]
#ang_mom[nr-1] = ang_mom[nr-2]
ax[1,0].plot(nr_r,nr_omega, 'rx')
